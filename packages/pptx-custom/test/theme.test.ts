...
import { applyCustomTheme, type PresentationData } from '../src/index'

function loadSlideFixture () {
  const raw = readFileSync(new URL('./assets/debug-source-1.json', import.meta.url), 'utf8')
  return JSON.parse(raw)
}

function loadFontColorFixture () {
  const raw = readFileSync(new URL('./assets/debug-font-color.json', import.meta.url), 'utf8')
  return JSON.parse(raw)
}
